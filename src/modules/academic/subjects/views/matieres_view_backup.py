...
class MatieresView(ctk.CTkFrame):
    ...
    def _build_header(self):
        ...
        header_content = ctk.CTkFrame(header_frame, fg_color=BG_SIDEBAR)
        header_content.pack(fill="x", padx=PADDING_CARD, pady=PADDING_CARD)
        
        # Section gauche - Titre et description
        left_section = ctk.CTkFrame(header_content, fg_color=BG_SIDEBAR)
        left_section.pack(side="left", fill="x", expand=True)
        
        title_container = ctk.CTkFrame(left_section, fg_color=BG_SIDEBAR)
        ...
        if main_icon:
            ctk.CTkLabel(title_container, text="", image=main_icon, 
                        fg_color=BG_SIDEBAR).pack(side="left", padx=(0, MARGIN_MEDIUM))
        else:
            ctk.CTkLabel(title_container, text="📚", font=FONT_TITLE, 
                        text_color=TEXT_PRIMARY, fg_color=BG_SIDEBAR).pack(side="left", padx=(0, MARGIN_MEDIUM))
        
        # Titre
        title_text = ctk.CTkLabel(title_container, text="Matières du Système Éducatif Guinéen",
                                 font=FONT_TITLE, text_color=TEXT_PRIMARY, fg_color=BG_SIDEBAR)
        title_text.pack(side="left")
        
        # Description
        desc_text = ctk.CTkLabel(left_section, text="Organisées par niveaux : Primaire, Collège, Lycée",
                                font=FONT_SECONDARY, text_color=TEXT_SECONDARY, fg_color=BG_SIDEBAR)
        desc_text.pack(anchor="w", pady=(MARGIN_SMALL, 0))
        
        # Section droite - Actions
        right_section = ctk.CTkFrame(header_content, fg_color=BG_SIDEBAR)
    ...
    def _build_navigation_panel(self):
        ...
        nav_content = ctk.CTkFrame(nav_frame, fg_color=CARD_BG)
        nav_content.pack(fill="x", padx=PADDING_CARD, pady=PADDING_CARD)
        
        # Filtres en ligne
        filters_frame = ctk.CTkFrame(nav_content, fg_color=CARD_BG)
        ...
        search_inner = ctk.CTkFrame(search_frame, fg_color=BG_MAIN)
        ...
        if search_icon:
            ctk.CTkLabel(search_inner, text="", image=search_icon, 
                        fg_color=BG_MAIN).pack(side="left", padx=(0, MARGIN_SMALL))
    ...
    def _build_content_area(self):
        ...
        # Conteneur principal
        content_container = ctk.CTkFrame(self, fg_color=BG_MAIN)
    ...
    def _build_stats_section(self):
        ...
        stats_content = ctk.CTkFrame(self.stats_frame, fg_color=CARD_BG)
        stats_content.pack(fill="x", padx=PADDING_CARD, pady=PADDING_CARD)
        
        # Statistiques principales
        stats_left = ctk.CTkFrame(stats_content, fg_color=CARD_BG)
        ...
        if stats_icon:
            ctk.CTkLabel(stats_left, text="", image=stats_icon, 
                        fg_color=CARD_BG).pack(side="left", padx=(0, MARGIN_MEDIUM))
    ...
    def _build_subjects_area(self):
        ...
        self.subjects_area = ctk.CTkScrollableFrame(self, fg_color=BG_MAIN)
    ...
    def _create_level_header(self, level: str) -> ctk.CTkFrame:
        ...
        content = ctk.CTkFrame(header, fg_color=ACCENT)
    ...
    def _create_grade_header(self, grade: str, subject_count: int) -> ctk.CTkFrame:
        ...
        content = ctk.CTkFrame(header, fg_color=SUCCESS_GREEN)
    ...
    def _render_empty_state(self):
        ...
        if empty_icon:
            ctk.CTkLabel(empty_frame, text="", image=empty_icon, 
                        fg_color=CARD_BG).pack(pady=(MARGIN_HERO, MARGIN_LARGE))
    ...
    def charger_matieres(self, q=""):
        ...
        if q.strip():
            self.var_search.set(q)
            self._search_subjects(q)
        else:
            self._load_all_subjects()

    def ajouter_matiere(self):
        """Méthode de compatibilité - ouvre le formulaire d'ajout"""
        messagebox.showinfo("Information", "Pour ajouter une matière personnalisée, utilisez le contrôleur guinéen directement.")

    def modifier_matiere(self, m):
        """Méthode de compatibilité - ouvre le formulaire de modification"""
        messagebox.showinfo("Information", "Pour modifier une matière, utilisez le contrôleur guinéen directement.")

    def supprimer_matiere(self, mid):
        """Méthode de compatibilité - supprime une matière"""
        if messagebox.askyesno("Confirmation", f"Voulez-vous vraiment désactiver cette matière ?"):
            try:
                if self.controller.deactivate_subject(mid):
                    self._load_all_subjects()
                    messagebox.showinfo("Succès", "Matière désactivée avec succès.")
                else:
                    messagebox.showerror("Erreur", "La désactivation a échoué.")
            except Exception as e:
                messagebox.showerror("Erreur", f"Erreur lors de la désactivation : {e}")

# Instance globale pour l'utilisation dans l'application
def create_matieres_view(parent, icons=None):
    """Factory function pour créer la vue des matières guinéennes"""
    return MatieresView(parent, icons)
