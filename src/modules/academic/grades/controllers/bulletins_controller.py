"""
Contrôleur principal pour la gestion des bulletins
...
class BulletinsController:
    ...
    def __init__(self, db_path: str):
        ...
        self.bulletin_model = BulletinAdvancedModel(db_path)
        self.calcul_controller = CalculBulletinController(db_path)
    ...
    def get_bulletin_eleve(self, id_eleve: int, id_periode: int) -> Optional[BulletinAvance]:
        ...
        return BulletinAvance(
            ...
            date_creation=datetime.strptime(row[18], '%Y-%m-%d %H:%M:%S'),
            date_modification=datetime.strptime(row[19], '%Y-%m-%d %H:%M:%S'),
            cree_par=row[20],
            valide_par=row[21],
            date_validation=datetime.strptime(row[22], '%Y-%m-%d %H:%M:%S') if row[22] else None
        )
    ...
    def get_top_eleves(self, limite: int = 10, id_periode: int = None) -> List[Dict]:
        ...
        query += " ORDER BY b.moyenne_generale DESC LIMIT ?"
    ...
    def valider_bulletin(self, bulletin_id: int, valide_par: str) -> bool:
        ...
        cursor.execute("""
            ...
            SET status = 'valide', valide_par = ?, date_validation = ?, date_modification = ?
            WHERE id = ?
        """, (valide_par, datetime.now(), datetime.now(), bulletin_id))
    ...
    def approuver_bulletin(self, bulletin_id: int, approuve_par: str) -> bool:
        ...
        cursor.execute("""
            UPDATE bulletins_avances 
            SET status = 'approuve', valide_par = ?, date_validation = ?, date_modification = ?
            WHERE id = ?
        """, (approuve_par, datetime.now(), datetime.now(), bulletin_id))
    ...
    def archiver_bulletin(self, bulletin_id: int) -> bool:
        ...
        cursor.execute("""
            UPDATE bulletins_avances 
            SET status = 'archive', date_modification = ?
            WHERE id = ?
        """, (datetime.now(), bulletin_id))
